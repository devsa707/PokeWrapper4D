...
begin
  ...
  begin
    ...
    try
      ...
      FPokeWrapper.GetAsEntity(LItemAttributeEntity,
        integer(TItem.item_attribute), I);
      // Assertions
      // node Descriptions
      Assert.IsNotEmpty(LItemAttributeEntity.descriptions.Items
        [LItemAttributeEntity.descriptions.Count - 1].description);
      Assert.IsNotEmpty(LItemAttributeEntity.descriptions.Items
        [LItemAttributeEntity.descriptions.Count - 1].language.name);
      Assert.IsNotEmpty(LItemAttributeEntity.descriptions.Items
        [LItemAttributeEntity.descriptions.Count - 1].language.url);
      //
      Assert.IsNotEmpty(LItemAttributeEntity.id);
      // node Nullable Items
      if LItemAttributeEntity.Items.Count > 0 then
      begin
        Assert.IsNotEmpty(LItemAttributeEntity.Items.Items
          [LItemAttributeEntity.Items.Count - 1].name);
        Assert.IsNotEmpty(LItemAttributeEntity.Items.Items
          [LItemAttributeEntity.Items.Count - 1].url);
      end;
      //
      Assert.IsNotEmpty(LItemAttributeEntity.name);
      // node Languages
      Assert.IsNotEmpty(LItemAttributeEntity.names.Items
        [LItemAttributeEntity.names.Count - 1].language.name);
      Assert.IsNotEmpty(LItemAttributeEntity.names.Items
        [LItemAttributeEntity.names.Count - 1].language.url);
      Assert.IsNotEmpty(LItemAttributeEntity.names.Items
        [LItemAttributeEntity.names.Count - 1].name);
      Write('.');
    finally
...
begin
  ...
  try
    LItemAttributeEntity := TItemAttributeEntity.Create;
    FPokeWrapper.GetAsEntity(LItemAttributeEntity,
      integer(TItem.item_attribute), 9999999);
  finally
    LItemAttributeEntity.Free;
  end;
end;
